	...
	query.find({
		success: function(results) {
			...
			$('#SCLdt tbody').on('click', 'a#edit', function() {
				...
				tableStr = "";
				for(var j = 1; j < robj.length; j++) {
					tableStr = tableStr + "<tr><td align='center' rowspan='2'>" + robj[j]['name'] +
						"</td>" + "<td align='center' rowspan='2'>" + robj[j]['total'] + "</td>" +
						"<td align='center' rowspan='2'>" + robj[j]['totalavg'] + "</td>";
					//得到题号
					var qarray = new Array(13);
					qarray.fill("");
					var qid = robj[j]['testid'].split(" ");
					for(var m = 0; m < qid.length; m++) {
						qarray[m] = qid[m];
					}
					//得到计分
					var aarray = new Array(13);
					aarray.fill("");
					var aid = robj[j]['score'].split(",");
					for(var n = 0; n < aid.length; n++) {
						aarray[n] = aid[n];
					}
					//填充表格
					tableStr = tableStr + "<td align='center'>" + "题号" + "</td>"
					for(var m = 0; m < qarray.length; m++) {
						tableStr = tableStr + "<td align='center'>" + qarray[m] + "</td>"
					}
					tableStr = tableStr + "</tr><tr>";
					tableStr = tableStr + "<td align='center'>" + "计分" + "</td>"
					
					for(var m = 0; m < aarray.length; m++) {
						tableStr = tableStr + "<td align='center'>" + aarray[m] + "</td>"
					}
				...
				$("#myModalLabel").text("测试日期："+object.get('testdate'));
				$('#myModal').modal();
